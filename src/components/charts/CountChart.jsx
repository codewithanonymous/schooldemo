...
const CountChart = () => {
  ...
  const chartData = [
    { name: 'Total', count: total, fill: 'var(--border-color)' },
    { name: 'Girls', count: girls, fill: 'var(--warning)' },
    { name: 'Boys',  count: boys,  fill: 'var(--primary)' },
  ]
  ...
  return (
    <div className="chart-card" style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: '380px' }}>
      ...
      <div style={{ flexGrow: 1, position: 'relative', width: '100%', minHeight: '200px' }}>
        <ResponsiveContainer>
          <RadialBarChart cx="50%" cy="50%" innerRadius="40%" outerRadius="100%" barSize={24} data={chartData}>
        ...
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '32px', fontWeight: 800, marginTop: '-4px' }}>{total}</h2>
        </div>
      </div>

      <div style={{ display: 'flex', justifyContent: 'center', gap: '32px', marginTop: '16px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div style={{ width: '12px', height: '12px', borderRadius: '50%', backgroundColor: 'var(--primary)', marginBottom: '4px' }} />
          <span style={{ fontSize: '13px', fontWeight: 600 }}>{boys}</span>
          <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Boys ({boysPercentage}%)</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div style={{ width: '12px', height: '12px', borderRadius: '50%', backgroundColor: 'var(--warning)', marginBottom: '4px' }} />
          <span style={{ fontSize: '13px', fontWeight: 600 }}>{girls}</span>
