...
import { STUDENTS as initialStudents, PARENTS, CLASSES, GRADES, genId } from '../../data/mockData'
...
const emptyForm = () => ({
  ...
  parent_id: PARENTS[0]?.id ?? '', class_id: CLASSES[0]?.id ?? '', grade_id: GRADES[0]?.id ?? ''
})
...
const Students = () => {
  ...
  const openEdit = (s) => {
    setModalType('edit'); setSelected(s)
    setForm({ ...s, password: '', parent_id: s.parent_id ?? '', class_id: s.class_id ?? '', grade_id: s.grade_id ?? '' })
    setShowModal(true)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (modalType === 'create') {
      setStudents(prev => [...prev, { ...form, id: genId('std'), class_id: form.class_id }])
    ...
    <tr key={s.id}>
      ...
      <td>
        <div className="table-actions">
          ...
          <button className="btn-icon btn-icon-danger" onClick={() => openDelete(s)}><Trash2 size={14} /></button>
        </div>
      </td>
    </tr>
  )
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="form-grid">
                  <div className="form-group"><label className="form-label">Gender</label>
                    <select className="form-select" value={form.sex} onChange={f('sex')}><option value="MALE">Male</option><option value="FEMALE">Female</option></select>
                  </div>
                  <div className="form-group"><label className="form-label">Blood Type</label>
                    <select className="form-select" value={form.blood_type} onChange={f('blood_type')}>{BLOOD_TYPES.map(bt => <option key={bt} value={bt}>{bt}</option>)}</select>
                  </div>
                </div>
                <div className="form-grid">
