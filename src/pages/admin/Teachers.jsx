...
const Teachers = () => {
  ...
  const fetchTeachers = async () => {
    setLoading(true)
    try {
      // RLS already scopes by school — the .eq is belt-and-suspenders for clarity
      const { data, error } = await supabase
        .from('teachers')
        .select(`
          id, username, name, surname, email, phone,
          address, blood_type, sex, birthday, created_at,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalType === 'create') {
        // 1. Create auth user + profile via secure Edge Function (role set server-side)
        ...
        // 2. teacher row is already inserted by set_user_profile — update extra fields
        ...
        // 3. Upsert teacher_subjects (include school_id per new schema)
        if (form.subject_ids.length > 0) {
          const rows = form.subject_ids.map(sid => ({
            teacher_id: userId,
            subject_id: sid,
            school_id:  schoolId
          }))
        ...
        // Update teacher row (no password change here — use Supabase dashboard for that)
        const { error: tError } = await supabase
          .from('teachers')
          .update({
            username:   form.username,
        ...
        await supabase.from('teacher_subjects').delete().eq('teacher_id', selected.id)
        if (form.subject_ids.length > 0) {
          const rows = form.subject_ids.map(sid => ({ teacher_id: selected.id, subject_id: sid }))
          const { error: tsError } = await supabase.from('teacher_subjects').insert(rows)
          if (tsError) throw tsError
        }
      }
      ...
      console.error('[Teachers] save error:', err.message)
  ...
  const handleDelete = async () => {
    ...
    setSaving(true)
    try {
      // Deleting the profile cascades to teachers row via FK
      ...
      console.error('[Teachers] delete error:', err.message)
      setFormError(err.message)
    } finally {
      setSaving(false)
    }
  ...
  const filtered = teachers.filter(t =>
    `${t.name} ${t.surname} ${t.username} ${t.email ?? ''}`.toLowerCase()
    ...
    <tr key={t.id}>
      ...
      <td>
        <div className="table-actions">
          <button className="btn-icon" onClick={() => openEdit(t)} aria-label="Edit teacher"><Edit2 size={14} /></button>
          <button className="btn-icon btn-icon-danger" onClick={() => openDelete(t)} aria-label="Delete teacher"><Trash2 size={14} /></button>
        </div>
      </td>
    </tr>
  )
      ...
      <div className="table-card">
        <div className="table-header-container">
          <div style={{ position: 'relative' }}>
            ...
            <input
              className="search-input" style={{ paddingLeft: 40 }}
      ...
      {showModal && (
        <div className="modal-overlay" role="dialog" aria-modal="true">
          <div className={`modal-content ${modalType === 'delete' ? 'confirm' : ''}`}>
            ...
            {modalType === 'delete' ? (
              <div>
                <p style={{ marginBottom: 24 }}>Delete <strong>{selected?.name} {selected?.surname}</strong>? This removes their account and all related records and cannot be undone.</p>
